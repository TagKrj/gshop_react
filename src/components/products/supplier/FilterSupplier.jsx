...
const FilterSupplier = ({ isOpen, onClose, onApplyFilter, supplierFilter = '', onSupplierClick, showSupplierList }) => {
    ...
    // Update filterData when supplierFilter prop changes
    useEffect(() => {
        if (supplierFilter !== filterData.supplier) {
            setFilterData(prev => ({
                ...prev,
                supplier: supplierFilter
            }));
    ...
    const renderDropdown = (label, name, value, placeholder = "Chọn", isSupplier = false) => (
        <div className="flex flex-col gap-2 w-full">
            <label className="text-sm font-semibold text-[#737373]">{label}</label>
            <div
                className={`flex justify-between items-center w-full border border-[#E5E5E5] rounded-[8px] p-4 ${isSupplier ? 'cursor-pointer' : ''}`}
                onClick={isSupplier ? (e) => onSupplierClick(e) : undefined}
            >
        ...
        >
            ...
            <div className="flex flex-col px-5 py-3 gap-6 overflow-y-auto">
                ...
                <div className="flex flex-col gap-3">
                    ...
                    {renderDropdown("Nhà cung cấp", "supplier", filterData.supplier, "Chọn", true)}
                    {renderDropdown("Người liên hệ", "contactPerson", filterData.contactPerson)}
                </div>
